...
/**
 * Base class that represents a row from the '_application' table.
 *
 *
 *
...
abstract class Application implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\Model\\Map\\ApplicationTableMap';
    ...
    /**
     * Initializes internal state of App\Model\Base\Application object.
     */
     ...
     * @return $this|\App\Model\Application The current object (for fluent API support)
     */
    ...
    public function setToken($v)
    {
        ...
        if ($this->token !== $v) {
            $this->token = $v;
            $this->modifiedColumns[ApplicationTableMap::COL_TOKEN] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ApplicationTableMap::translateFieldName('Token', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = ApplicationTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\App\\Model\\Application'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ApplicationTableMap::COL_TOKEN)) {
            $modifiedColumns[':p' . $index++]  = 'token';
        }

        $sql = sprintf(
            'INSERT INTO _application (%s) VALUES (%s)',
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\Model\Application The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ApplicationTableMap::COL_TOKEN)) {
            $criteria->add(ApplicationTableMap::COL_TOKEN, $this->token);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\Model\Application (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\Model\Application Clone of current object.
     * @throws PropelException
     */
    ...
    public function setSessions(Collection $sessions, ConnectionInterface $con = null)
    {
        ...
        foreach ($sessionsToDelete as $sessionRemoved) {
            $sessionRemoved->setApplication(null);
        }
