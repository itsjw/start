...
/**
 * Base class that represents a row from the '_resource' table.
 *
 *
 *
...
abstract class Resource implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\App\\Model\\Map\\ResourceTableMap';
    ...
    /**
     * Initializes internal state of App\Model\Base\Resource object.
     */
     ...
     * @return $this|\App\Model\Resource The current object (for fluent API support)
     */
    ...
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[ResourceTableMap::COL_VALUE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ResourceTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = ResourceTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\App\\Model\\Resource'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('_resource_id_seq')");
        ...
        if ($this->isColumnModified(ResourceTableMap::COL_VALUE)) {
            $modifiedColumns[':p' . $index++]  = 'value';
        }

        $sql = sprintf(
            'INSERT INTO _resource (%s) VALUES (%s)',
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\App\Model\Resource
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\App\Model\Resource The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ResourceTableMap::COL_VALUE)) {
            $criteria->add(ResourceTableMap::COL_VALUE, $this->value);
        }
     ...
     * objects.
     *
     * @param      object $copyObj An object of \App\Model\Resource (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \App\Model\Resource Clone of current object.
     * @throws PropelException
     */
