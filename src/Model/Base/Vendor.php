...
abstract class Vendor implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Perfumerlabs\\Start\\Model\\Map\\VendorTableMap';
    ...
    /**
     * Initializes internal state of Perfumerlabs\Start\Model\Base\Vendor object.
     */
    ...
    public function setHostname($v)
    {
        ...
        if ($this->hostname !== $v) {
            $this->hostname = $v;
            $this->modifiedColumns[VendorTableMap::COL_HOSTNAME] = true;
        }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->activitiesScheduledForDeletion !== null) {
                if (!$this->activitiesScheduledForDeletion->isEmpty()) {
                    foreach ($this->activitiesScheduledForDeletion as $activity) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {
                $dataFetcher = $con->query("SELECT nextval('vendor_id_seq')");
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Perfumerlabs\Start\Model\Vendor
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Perfumerlabs\Start\Model\Vendor The current object, for fluid interface
     */
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Perfumerlabs\Start\Model\Vendor (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Perfumerlabs\Start\Model\Vendor Clone of current object.
     * @throws PropelException
     */
    ...
    public function initActivities($overrideExisting = true)
    {
        ...
        $this->collActivities->setModel('\Perfumerlabs\Start\Model\Activity');
    }
    ...
    public function setActivities(Collection $activities, ConnectionInterface $con = null)
    {
        ...
        foreach ($activitiesToDelete as $activityRemoved) {
            $activityRemoved->setVendor(null);
        }
